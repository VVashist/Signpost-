    ...
    private int makeForwardConnections(Model model) {
        ...
        for (Sq sq : model) {
            ...
            if (sq.successor() == null && sq.direction() != 0) {
                nFound = sq.successors().size();
                for (Place p : _successorCells[sq.x][sq.y][sq.direction()]) {

                    found = model.get(p);
                    if (sq.sequenceNum() != 0 && found.sequenceNum() != 0) {
                        if (sq.sequenceNum() == found.sequenceNum() - 1) {
                            nFound = 1;
                            break;
                        }
                    }
    ...
    private int makeBackwardConnections(Model model) {
        ...
        for (Sq sq : model) {
            ...
            if (sq.predecessor() == null && sq.sequenceNum() != 1) {
                nFound = sq.predecessors().size();
                for (Place p : sq.predecessors()) {
                    found = model.get(p);
                    if (found.connectable(sq)) {
                        if (sq.sequenceNum() != 0 && found.sequenceNum() != 0) {
                            nFound = 1;
                            break;
                        }
